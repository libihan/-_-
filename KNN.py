# -*- coding: utf-8 -*-
"""
Created on Mon Nov  6 19:27:07 2017
...
def get_hog_features(trainset):
    ...
    hog = cv2.HOGDescriptor('../hog.xlml')
...
def Predict(testset,trainset,train_labels):
    ...
    for test_vec in testset:
        ...
        # 剩下的点，每次都判断该i位置是否放入knn_list中。最后得到最终的knn_list。
        for i in range(k,len(train_labels)):
            ...
            '''如果当前k个最近邻居中存在点距离比当前点距离远，则替换
               丢弃当前的max_dist位置，将其替换成i位置的数，然后复位（重新寻找距离最远的点）；否则继续扫描下一个位置
            '''
            if dist < max_dist:
		...
		#找出knn_list最大标签  法2：先放入dic(label,count);然后max(dic,key = lambda x:dic[x])
        # 统计选票
        class_total = 10
